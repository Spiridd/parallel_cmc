// TODO MPI_STATUS_IGNORE
// TODO provide non contiguous I/O (using derived types)
// TODO replace seek calls with _at functions
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
#include <time.h>
...
void read_matrices_from_files(MPI_Comm cart_comm, const char *filename_A, const char *filename_B, double **A_ptr, double **B_ptr, int *i0_ptr, int *i1_ptr, int *j0_ptr, int *j1_ptr, int *k0_ptr, int *k1_ptr, int n)
{
    ...
    *B_ptr = safe_malloc((j1-j0)*(k1-k0)*sizeof(double));
    ...
    // skip 3 ints (type, size, size)
    MPI_File_set_view(fh_A, 3*sizeof(int), MPI_DOUBLE, MPI_DOUBLE, "native", MPI_INFO_NULL);
    const int width_A = k1-k0;
    for(int i=i0; i<i1; ++i)
    {
        // seek from the beginning of the view (may be slow)
        MPI_File_seek(fh_A, i*n+k0, MPI_SEEK_SET);
        MPI_File_read(fh_A, A+width_A*(i-i0), width_A, MPI_DOUBLE, MPI_STATUS_IGNORE);
    }
    MPI_File_close(&fh_A);
    ...
    {
        // seek from the beginning of the view (may be slow)
        MPI_File_seek(fh_B, k*n+j0, MPI_SEEK_SET);
        MPI_File_read(fh_B, B+width_B*(k-k0), width_B, MPI_DOUBLE, MPI_STATUS_IGNORE);
    }
    MPI_File_close(&fh_B);
}
...
void parallel_multiply(const char *filename_A, const char *filename_B, const char *filename_C)
{
    ...
    read_matrices_from_files(cart_comm, filename_A, filename_B, &A, &B, &i0, &i1, &j0, &j1, &k0, &k1, n);
    ...
    if (rank == 0)  write_matrix_in_file(filename_C, global_C, i0, i1, j0, j1, n, square_comm);
